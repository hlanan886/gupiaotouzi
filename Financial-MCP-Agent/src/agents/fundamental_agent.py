...
from src.utils.context_config import get_data_retrieval_limit
...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # 根据模型上下文窗口动态调整 max_tokens
        from src.utils.context_config import get_agent_max_tokens
        dynamic_max_tokens = get_agent_max_tokens(model_name, "fundamental")
        logger.info(
            f"FundamentalAgent: Context-aware max_tokens={dynamic_max_tokens} for model {model_name}")
        ...
        try:
            ...
            # 根据上下文窗口动态生成数据获取限制
            retrieval_limits = get_data_retrieval_limit(model_name, "fundamental")
...
请进行以下基本面分析：
1. 获取公司基本信息和行业背景
2. 获取最新一个季度的财务报表数据（资产负债表、利润表、现金流量表）
...
重要限制：
- {retrieval_limits['additional_instructions']}
- 只获取最新一个季度的财务数据（如2025Q4或2026Q1），不要获取多个季度的数据
- 不要获取历史分红明细，只需获取分红概况即可
- 每个工具只调用一次
- 请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请基于可用信息提供简要分析。"""
