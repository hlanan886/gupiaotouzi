...
async def summary_agent(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        **重要提醒：**
        ...
        review_feedback = review_scores.get("detailed_feedback", "")
        review_attempts = state.get("review_attempts", 0)

        if review_feedback:
            user_prompt += f"""

        【第{review_attempts + 1}次生成 - 评审反馈】
        上次评审总分: {review_scores.get("total_score", 0)}/100
        请根据以下评审意见改进你的报告：
        {review_feedback}
        """
        ...
        # 如果是重试，复用已有的report_path（覆盖旧文件）；否则创建新文件
        existing_path = current_data.get("report_path")
        if review_feedback and existing_path and os.path.exists(existing_path):
            # 重试：覆盖原有文件
            report_path = existing_path
            logger.info(f"{WAIT_ICON} SummaryAgent: Overwriting existing report at {report_path}")
        else:
            # 首次生成：使用时间戳命名
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            report_filename = f"{safe_file_prefix}_{timestamp}.md"
            report_path = os.path.join(reports_dir, report_filename)
            logger.info(f"{WAIT_ICON} SummaryAgent: Creating new report at {report_path}")
        ...
        if review_feedback and existing_path and os.path.exists(existing_path):
            try:
                with open(existing_path, "r", encoding="utf-8") as f:
                    old_report = f.read()
                # 保存到执行日志的reports子目录，带版本号
                version_dir = execution_logger.execution_dir / "report_versions"
                version_dir.mkdir(exist_ok=True)
                version_file = version_dir / f"version_{review_attempts}.md"
                with open(version_file, "w", encoding="utf-8") as f:
                    f.write(old_report)
                logger.info(f"SummaryAgent: Saved previous report version to {version_file}")
            except Exception as e:
                logger.warning(f"SummaryAgent: Failed to save previous version: {e}")

        # 将报告写入文件
        with open(report_path, "w", encoding="utf-8") as f:
            f.write(final_report)

        logger.info(f"{SUCCESS_ICON} SummaryAgent: Report saved to {report_path}")
        ...
        # 递增重试计数
        new_attempts = (review_attempts + 1) if review_feedback else 0
