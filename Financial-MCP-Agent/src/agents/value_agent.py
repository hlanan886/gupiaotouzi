...
async def value_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        dynamic_max_tokens = get_agent_max_tokens(model_name, "value")
        logger.info(
            f"ValueAgent: Context-aware max_tokens={dynamic_max_tokens} for model {model_name}")
        ...
        try:
            ...
            # 根据上下文窗口动态生成数据获取限制
            retrieval_limits = get_data_retrieval_limit(model_name, "value")
