...
工作流程：
start_node → [fundamental_analyst, technical_analyst, value_analyst] → summarizer → reviewer → END
"""
...
from src.agents.reviewer_agent import review_agent      # 评审智能体：评估报告质量
...
async def main():
    ...
    try:
        ...
        workflow.add_node("reviewer", review_agent)                 # 评审智能体
        ...
        def route_after_review(state):
            """评分>=80则结束，否则重新生成（最多2次生成）"""
            review_scores = state.get("review_scores", {})
            total_score = review_scores.get("total_score", 0)
        ...
        initial_state = AgentState(
            ...
            metadata={},  # 其他运行时特定信息
            review_scores={},  # 评审得分（初始为空）
            review_attempts=0,  # 评审重试计数（初始为0）
        )
